...
- [ ] add `systemInstruction` field (`Content` type, `@JsonInclude(NON_NULL)`) to `GeminiRequest`
- [ ] add `role` field (`String`, `@JsonInclude(NON_NULL)`) to `GeminiRequest.Content`
- [ ] verify existing tests still compile (no behavior change yet)
- [ ] run tests: `./gradlew :cookbook:test` — must pass before task 2
